...
INSERT INTO Motosiklet (vasita_id,zamanlama_tipi,silindir_sayisi,sogutma,olusturulma_tarihi,guncellenme_tarihi) 
VALUES (last_insert_id(), "Çift Zamanlı", 6, "Su soğutmalı", NOW(), NOW());

COMMIT;  

-- MOTO SELECT

SELECT * FROM motosiklet 
...
-- UPDATE (motosiklet,vasita_ilan,ilan,adres,kategori,model,seri,marka) .... 
UPDATE motosiklet
INNER JOIN vasita_ilan ON vasita_ilan.vasita_id = motosiklet.vasita_id
INNER JOIN ilan ON vasita_ilan.ilan_id = ilan.ilan_id
INNER JOIN adres ON ilan.adres_id = adres.adres_id
