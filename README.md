...
* **Kullanıcı Yönetimi:** Güvenli kayıt, giriş (hashlenmiş şifreler) ve profil yönetimi[cite: 160, 165].
* **İlan Yönetimi:**
    * **Vasıta:** Otomobil, Motosiklet, Karavan ve Tır gibi alt kategorilerde detaylı özellik girişi[cite: 50, 363].
    * **Emlak:** Konut, Arsa, Turistik Tesis gibi türlere özel veri alanları[cite: 60, 297].
* **Gelişmiş Arama ve Filtreleme:** Kategori, şehir, fiyat aralığı ve teknik özelliklere (vites tipi, m², oda sayısı vb.) göre dinamik filtreleme[cite: 199, 226].
* **Favoriler:** Kullanıcıların ilgilendikleri ilanları favori listelerine ekleyebilmesi[cite: 37].
* **Güvenlik:** Yetkilendirme (Authorization) ve sahiplik (Ownership) kontrolleri[cite: 174].

## 🛠 Teknoloji Yığını

Proje, **Backend** ve **Frontend** olmak üzere iki ana modülde geliştirilmiştir[cite: 1089].

### Backend (Python & Flask)
* **Framework:** Flask (MVC mimarisi: Controller -> Service -> Router)[cite: 1092, 1153].
* **ORM/Veritabanı Bağlantısı:** SQLAlchemy, PyMySQL[cite: 1096].
* **Güvenlik:** Flask-Bcrypt (Şifre hashleme), Flask-Session[cite: 1139, 1138].

### Frontend (Flutter)
* **SDK:** Flutter (Web tabanlı uygulama)[cite: 1192].
* **Dil:** Dart.
* **Routing:** go_router[cite: 1208].
* **HTTP İstekleri:** http paketi[cite: 1208].

### Veritabanı (MySQL)
* **VTYS:** MySQL / InnoDB Motoru[cite: 16, 1108].
* **Tasarım:** 3NF ve BCNF prensiplerine tam uyumluluk[cite: 14, 1116].

## 🗄 Veritabanı Mimarisi

Veritabanı tasarımı, karmaşık veri yapılarını yönetmek için **Genelleştirme/Özelleştirme (Supertype/Subtype)** modelini kullanır[cite: 1082].


* **Inheritance (Kalıtım):** `ILAN` tablosu ana varlıktır. `VASITA` ve `EMLAK` tabloları bu tablodan türer. Örneğin bir otomobil ilanı için hiyerarşi şöyledir: `ILAN` -> `VASITA` -> `OTOMOBIL`[cite: 1082, 1085].
* **Kısıtlamalar (Constraints):**
    * Veri bütünlüğü için `ON DELETE CASCADE` kullanılmıştır[cite: 131].
    * Mantıksal kontroller (Örn: $m2\_net \le m2\_brut$ veya $fiyat \ge 0$)[cite: 101, 107].
* **İndeksleme:** Performans için `kategori_isim`, `fiyat`, `il_ilce` gibi sık sorgulanan alanlarda B-Tree indeksleme yapılmıştır[cite: 1115].
