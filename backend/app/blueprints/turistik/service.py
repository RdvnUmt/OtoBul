...
def add_service(data,statement_list):
    ...
    with engine.connect() as con:
        ...
        try: 
            ...
            smt = text(f"""SELECT last_insert_id() AS id""")
            for row in con.execute(smt):
                turistik_tesis_id = row
            print(turistik_tesis_id)
            data['turistik_id'] = turistik_tesis_id[0]
    ...
    return "turistik_tesis başarıyla oluşturuldu",200    
...
def delete_service(data,statement):
    ...
    return "turistik_tesis başarıyla silindi",200   
...
def update_service(data,statement):
    ...
    return "turistik_tesis başarıyla güncellendi",200   
