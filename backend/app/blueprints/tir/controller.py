...
def add_tir_controller(data):
    ...
    statement8 = text(f""" INSERT INTO tir(vasita_id,kabin,lastik_durumu_yuzde,yatak_sayisi,dorse,olusturulma_tarihi,guncellenme_tarihi) 
                    VALUES (:vasita_id,:kabin,:lastik_durumu_yuzde,:yatak_sayisi,:dorse,:olusturulma_tarihi,:guncellenme_tarihi) ; """)
...
def get_tir_controller(data):
    query_str,data = get_method_parser(data)
    print(query_str)
    print(data)
    
    statement = text(f""" SELECT * FROM tir 
                        INNER JOIN vasita_ilan ON vasita_ilan.vasita_id = tir.vasita_id
                        INNER JOIN ilan ON vasita_ilan.ilan_id = ilan.ilan_id
                        INNER JOIN adres ON ilan.adres_id = adres.adres_id
                        INNER JOIN kategori ON kategori.kategori_id = ilan.kategori_id
