...
def add_otomobil_controller(data):
    ...
    statement8 = text(f""" INSERT INTO Otomobil(vasita_id,kasa_tipi,cekis,olusturulma_tarihi ,guncellenme_tarihi) 
                    VALUES (:vasita_id,:kasa_tipi,:cekis,:olusturulma_tarihi ,:guncellenme_tarihi); """)
...
def delete_otomobil_controller(data):
    ...
    statement = text(f""" DELETE ilan,kategori,adres, marka
                        ...
                        WHERE otomobil.otomobil_id = :otomobil_id;  """)
...
def update_otomobil_controller(data):
    ...
    statement = text(f""" UPDATE otomobil
                        INNER JOIN vasita_ilan ON vasita_ilan.vasita_id = otomobil.vasita_id
                        INNER JOIN ilan ON vasita_ilan.ilan_id = ilan.ilan_id
                        INNER JOIN adres ON ilan.adres_id = adres.adres_id
                        INNER JOIN kategori ON kategori.kategori_id = ilan.kategori_id
...
def get_otomobil_controller(data):

    query_str,data = get_method_parser(data)
    print(query_str)
    print(data)
    
    statement = text(f""" SELECT * FROM otomobil 
                        INNER JOIN vasita_ilan ON vasita_ilan.vasita_id = otomobil.vasita_id
                        INNER JOIN ilan ON vasita_ilan.ilan_id = ilan.ilan_id
                        INNER JOIN adres ON ilan.adres_id = adres.adres_id
                        INNER JOIN kategori ON kategori.kategori_id = ilan.kategori_id
