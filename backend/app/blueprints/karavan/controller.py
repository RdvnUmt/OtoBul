...
from app.blueprints.karavan.service import add_service,delete_service,update_service, get_service
from app.utils.utils import get_method_parser
from app.policies.policies import id_control_policy

def add_karavan_controller(data):
    ...
    statement8 = text(f""" INSERT INTO karavan(vasita_id,yatak_sayisi,karavan_turu,karavan_tipi,olusturulma_tarihi ,guncellenme_tarihi) 
                    VALUES (:vasita_id,:yatak_sayisi,:karavan_turu,:karavan_tipi,:olusturulma_tarihi,:guncellenme_tarihi); """)
...
def delete_karavan_controller(data):

    if not (id_control_policy("karavan", "kara_id", data)):
...
def update_karavan_controller(data):
    ...
    statement = text(f""" UPDATE karavan
                        INNER JOIN vasita_ilan ON vasita_ilan.vasita_id = karavan.vasita_id
                        INNER JOIN ilan ON vasita_ilan.ilan_id = ilan.ilan_id
                        INNER JOIN adres ON ilan.adres_id = adres.adres_id
                        INNER JOIN kategori ON kategori.kategori_id = ilan.kategori_id
...
def get_karavan_controller(data):

    query_str,data = get_method_parser(data)
    print(query_str)
    print(data)
    
    statement = text(f""" SELECT * FROM karavan 
                        INNER JOIN vasita_ilan ON vasita_ilan.vasita_id = karavan.vasita_id
                        INNER JOIN ilan ON vasita_ilan.ilan_id = ilan.ilan_id
                        INNER JOIN adres ON ilan.adres_id = adres.adres_id
                        INNER JOIN kategori ON kategori.kategori_id = ilan.kategori_id
