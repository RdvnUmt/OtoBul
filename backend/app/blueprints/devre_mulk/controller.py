...
def add_devre_mulk_controller(data):
    ...
    statement6 = text(f""" INSERT INTO devre_mulk(yerleske_id ,donem ,olusturulma_tarihi,guncellenme_tarihi)
                    VALUES (:yerleske_id ,:donem ,:olusturulma_tarihi,:guncellenme_tarihi); """)
...
def delete_devre_mulk_controller(data):
    ...
    statement = text(f""" DELETE ilan,kategori,adres
                        ...
                        WHERE devre_mulk.devre_id = :devre_id;  """)
...
def update_devre_mulk_controller(data):
    ...
    statement = text(f""" UPDATE devre_mulk
                        INNER JOIN yerleske_ilan ON yerleske_ilan.yerleske_id = devre_mulk.yerleske_id
                        INNER JOIN emlak_ilan ON emlak_ilan.emlak_id = yerleske_ilan.emlak_id
                        INNER JOIN ilan ON emlak_ilan.ilan_id = ilan.ilan_id
                        INNER JOIN adres ON ilan.adres_id = adres.adres_id
