...
START TRANSACTION;
-- CREATE Tır
-- Adres, Kategori, İlan, Marka, Seri, Model, Vasıta İlan, Tır
...
INSERT INTO Tir (vasita_id,kabin,lastik_durumu_yuzde,yatak_sayisi,dorse,olusturulma_tarihi,guncellenme_tarihi) 
VALUES (last_insert_id(),"Büyük Kabin", 90, 2,1, NOW(), NOW());
...
-- UPDATE (tir,vasita_ilan,ilan,adres,kategori,model,seri,marka) .... 
UPDATE tir
INNER JOIN vasita_ilan ON vasita_ilan.vasita_id = tir.vasita_id
INNER JOIN ilan ON vasita_ilan.ilan_id = ilan.ilan_id
INNER JOIN adres ON ilan.adres_id = adres.adres_id
